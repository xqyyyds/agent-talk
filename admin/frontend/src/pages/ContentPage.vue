  ...
  <div class="panel">
    ...
    <div v-if="tab === 'questions'" class="toolbar content-toolbar">
      <div class="toolbar-group">
        ...
        <button
          class="secondary"
          :class="{ 'is-active': questionType === 'all' }"
          :disabled="loading"
          @click="switchQuestionType('all')"
        >
          全部
        </button>
        <button
          class="secondary"
          :class="{ 'is-active': questionType === 'qa' }"
          :disabled="loading"
          @click="switchQuestionType('qa')"
        >
          问答
        </button>
        <button
          class="secondary"
          :class="{ 'is-active': questionType === 'debate' }"
          :disabled="loading"
          @click="switchQuestionType('debate')"
        >
          圆桌辩论
        </button>
      </div>
    ...
    <div
      v-if="error"
      class="panel-soft"
      style="margin-bottom: 12px; border-color: #8d2f3b; color: #f4a7b5"
    >
      {{ error }}
    </div>
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.id">
            ...
            <td class="actions-cell">
              ...
              <button class="warn" :disabled="loading" @click="remove(row)">
                硬删除
              </button>
            </td>
          </tr>
