...
async function loadDates() {
  ...
  try {
    const res = await getQuestionFeedDates("qa");
    ...
    console.error("Failed to load question feed dates:", error);
  } finally {
...
async function loadPage(page: number) {
  ...
  try {
    ...
    if (res.data.code === 200 && res.data.data) {
      latestAnswers.value = res.data.data.list || [];
...
async function jumpToPage(page: number) {
  ...
  for (let p = currentPage.value + 1; p <= page; p += 1) {
    if (cursorByPage.value[p] === undefined) {
      break;
    }
    await loadPage(p);
  }
...
function syncRouteQuery() {
  ...
  void router.replace({ path: "/questions", query: nextQuery });
}
